...
public class GeneraPersones {
	...
	public static List<Usuari> gUsuaris(int nombre, List<Pais> paisos) {
		...
		for(int i=1; i<=nombre; i++) {
			...
			int numPais = rand.nextInt(lPaisos.size());
			Pais pais = lPaisos.get(numPais);
			String codiPais= pais.getISO();
			String nomPais= pais.getNomPais();

			//Adreça Codi Postal
			Integer randomCP=(int)(Math.random()*(47999-1)+1);
			//String cp = randomCP.toString();
						
			c = new Usuari(i, email, password, username, data, sex, codiPais, nomPais, randomCP);
	...
	public static List<Playlist> gPlaylists(int numPlayLists, int numUsuaris, int numVideos){
		...
		for(int i=0; i<numPlayLists; i++) {
			...
			for(int k=0; k<num; k++) {
				videos.add(rand.nextInt(numVideos)+1);
			}
			
			Playlist pl = new Playlist(i, creador, nom, creacio, publica, videos);
			playlists.add(pl);
